...
public class FixPoint {
	...
	public void execute(Event x) {
		...
		case CALL:// call(c, s) : // state is over the param of m, m is
					// MethodRef of call c
			i = x.getInst();
			s = x.getState();
			m = ((Call) i).getMethodRef();
			s0 = domain.extend(s, domain.bottom(m.getVariables())); // add local
																	// vars
			li = m.getFirstInsts();
		...
		case EXEC:// exec(i,s) :
			...
			case RETURN: // exec(i, s) // i is a return
				...
				for (Call cl : m.getCallPoints()) {
					...
					while (ii.hasNext())
						decideHandleRet(ii.next(), cl, (Return) i, s0);// Q.add(
																		// new
																		// Event(TypeEvent.EXEC,inst,
																		// s2)
																		// ));
			...
			case SYNCCALL: // exec(i, s) // i is a call
				...
				s1 = domain.rename(s0, lv2, lv);// m.actualParams,
												// m.formalParams);
	...
	public void decideHandleRet(GlatInstruction inst, Call cl, Return r, AbstractState s) {
		...
		s0 = domain.rename(s, lv, lv2);// m.formalParam()+m.retvar(),
										// m.actualParam()+m.retPoint());
