...
import glat.program.instructions.Assert;
...
import glat.program.instructions.Lock;
...
public class Mtanalysis {
	...
	public void analyse(Program p) {
		...
		while (!outInter.equals(inInter)) {
			...
			Ts.forEach((t) -> {
				...
				AnalyseThread(label, t.getMethod(), pc,
						states/* ,p,states,inInter,outInter */);
			});
		}
	}
	...
	public void AnalyseThread(String t, GlatMethod m, GlatNode pc, HashMap<Node, AbsState> states) {
		...
		while (!Q.isEmpty()) {
			...
			if (b) {
				m.getControlFlowGraph().getOutTransitions(tau.getTargetNode())
						.forEach((tr) -> Q.add((GlatTransition) tr));
			}
		}
